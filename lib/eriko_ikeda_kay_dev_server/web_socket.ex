defmodule ErikoIkedaKayDevServer.WebSocket do
  @moduledoc false
  ...
  def init(_opts) do
    ...
    {:ok,
     %{reloader: Application.get_env(:eriko_ikeda_kay, :reloader, patterns: [])}}
  end
  ...
  def handle_info({:file_event, watcher_pid, {path, event}}, state) do
    WebDevUtils.LiveReload.reload!(
      {:file_event, watcher_pid, {Path.relative_to_cwd(path), event}},
